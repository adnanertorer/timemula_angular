...
export class ArtPackageComponent implements OnInit {
  ...
  displayedColumns: string[] = ['categoryName', 'subCategoryName', 'lessonName', 'artPackageName', 'seanceCount',
  'duration', 'unitPrice', 'seancePrice', 'discount', 'participantTypeName', 'participantName', 'isActive', 'description', 'id'];
  ...
  constructor(private service: ArtPackageService, private categoryService: CategoryService, 
    private subCategoryService: SubCategoryService,
     private participantTypeService: ParticipantTypeService, private participantService:ParticipantService,
  ...
  ngOnInit() {
    ...
    this.service.getListAll().subscribe((data)=>{
      if(data.success){
        this.artPackages = data.dynamicClass as VArtPackageModel[];
  ...
  getSubCategories(id: number) {
    this.subCategoryService.getList(id).subscribe((data) => {
  ...
  getParticipantTypes(){
    this.participantTypeService.getList().subscribe((data)=>{
      if(data.success){
        this.participantTypes = data.dynamicClass as ParticipationModel[];
      }
    });
  }
  ...
  getParticipants(){
    this.participantService.getList().subscribe((data)=>{
      if(data.success){
        this.participants = data.dynamicClass as ParticipantModel[];
      }
    })
  }
  ...
  getDetailFromTable(resource: any): void {
    ...
    this.service.getDetails(id).subscribe((data)=>{
      if(data.success){
        ...
        this.subCategoryService.getList(this.artPackage.categoryId).subscribe((data) => {
          this.subCategories = data.dynamicClass as SubCategoryModel[];
          this.artPackage.subCategoryId = this.artPackage.subCategoryId;
          this.lessonService.getByCategory(this.artPackage.categoryId, this.artPackage.subCategoryId).subscribe((data)=>{
            if(data.success){
              this.lessons = data.dynamicClass as VLessons[];
