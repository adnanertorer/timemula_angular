...
export class AuthService implements OnDestroy{
    ...
    register(name: string, surname: string, email: string, password: string, address: string, phone: string){
        return this.http
        .post<RegisterResult>(`${this.apiUrl}AdminUser/CreateUser`, {name, surname, email, password, address, phone})
        .pipe(
            map((x) => {
                return x;
            })
    ...
    refreshToken(){
        ...
        return this.http
        .post<BaseResponse>(`${this.apiUrl}/AdminUser/RefreshToken`, refreshRequest)
        .pipe(
            map((x) => {
